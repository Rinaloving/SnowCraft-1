...
def throwSnowBall():  # snow ball is thrown
    ...
    # add snowball to list
    canvas.data.redSnow += [canvas.data.snowBallCounter]
...
def drawInstructions():  # instruction screen
    ...
    canvas.create_text(x/2, y/2, text=text,
                       font="Helvtica 10 bold", fill="blue")
...
def drawSnowball():  # draws snowball and shadow for both green and red
    ...
    for snowball in canvas.data.snow:  # for each snowball
        ...
        canvas.create_oval(l, t+shadowDistance, r, b +
                           shadowDistance, fill="black")
...
def drawSnowSplashes():  # at the end of the snowball's fall, the snowball makes
    ...
    for splash in canvas.data.snowSplashes:  # for each fallen snowball
        ...
        # splash image is
        canvas.create_image(x, y, anchor=N, image=snowSplash)
        ...
        for gSnow in canvas.data.greenSnow:  # check each snowball if it hit a
            ...
            if (l < 0) or (t < 0) or\
               (r > canvas.data.width) or (b > canvas.data.height):
                ...
                canvas.data.greenSnow.remove(
                    gSnow), randomizeCommand(abs(gSnow))
            elif (x-playerRadius) <= l and (y-playerRadius) <= t and\
...
def leftMousePressed(event):
    if canvas.data.paused == False:  # only if game is not puased
        ...
        playerRadius = canvas.data.playerRadius
        for player in range(len(canvas.data.redPlayersList)):  # check if player
            ...
            if event.x >= (x-playerRadius) and event.x <= (x+playerRadius) and\
               ...
               hitTime == 0:  # hit player cannot be selected
                canvas.data.playerSelected = [
                    True, x, y]  # store selected player
...
def leftMouseMoved(event):  # only if game is not paused and player is selected
    if canvas.data.paused == False and canvas.data.playerSelected[0] == True:
        ...
        if event.x+event.y <= canvas.data.width:
            xPlayer = canvas.data.width-yPlayer
        # player cannot move to enemy territory
        if event.x >= canvas.data.width:
            xPlayer = canvas.data.width-playerRadius
        # player may not move out of bounds
        if event.y >= canvas.data.width:
            yPlayer = canvas.data.width-playerRadius
...
def greenPlayerAI():  # the AI for the green players
    for player in range(len(canvas.data.greenPlayersList)):
        # green player is doing
        if canvas.data.greenPlayersList[player][3] > 0:
            pass  # something
        # green player is doing nothing
        elif canvas.data.greenOrders[player] == 0:
...
def moveGreen(player):  # moves green player in a given direction
    ...
    playerRadius = canvas.data.playerRadius
    if steps == 0:
        canvas.data.greenOrders[player] = 0
    else:
        ...
        if direction == "Left" and x-playerRadius > 0:
            x -= stepSize  # out of
        elif direction == "Up" and y-playerRadius > 0:
            y -= stepSize  # screen
        elif (x+playerRadius)+(y+playerRadius) != canvas.data.width:
            # player may not cross into enemy territory
            if direction == "Right" and x <= canvas.data.width:
                x += stepSize
            if direction == "Down" and y <= canvas.data.height:
                y += stepSize
...
def init():  # stores initial values
    ...
    if canvas.data.level == 1:
        initialRedPlayers(), initialGreenLevelOne()
...
def createRedPlayers(player):  # creates red players at given location
    ...
    # draw image for selected player
    elif canvas.data.playerSelected == [True, x, y]:
        canvas.create_image(x, y-playerRadius, anchor=N, image=rSelected)
    else:
        canvas.create_image(x, y-playerRadius, anchor=N, image=rStand)
